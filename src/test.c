...
void test_nn_resize()
{
    ...
    // 1.1
    save_image(resized, "output/dog4x-nn"); 
...
void test_bl_resize()
{
    ...
    // 1.2
    save_image(resized, "output/dog4x-bl"); 
    ...
    image thumbnail = bilinear_resize(im, im.w/7, im.h/7);
    // 1.3
    save_image(thumbnail, "output/dog7th_bl");
    free_image(thumbnail);
    free_image(im);
}
...
void test_highpass_filter(){
    ...
    // 2.3
    save_image(blur, "output/dog-highpass");
...
void test_emboss_filter(){
    ...
    // 2.4
    save_image(blur, "output/dog-emboss");
...
void test_sharpen_filter(){
    ...
    // 2.5
    save_image(blur, "output/dog-sharpen");
...
void test_convolution(){
    ...
    clamp_image(blur);
    // 2.1
    save_image(blur, "output/dog-box7");

    image gt = load_image("figs/dog-box7.png");
    TEST(same_image(blur, gt));

    image thumb = nn_resize(blur, blur.w/7, blur.h/7);
...
void test_gaussian_blur(){
    ...
    // 2.6
    save_image(blur, "output/dog-gauss2");
...
void test_frequency_image(){
    ...
    // 2.7
    save_image(lfreq, "output/low-frequency");
    save_image(hfreq, "output/high-frequency");
    save_image(reconstruct, "output/reconstruct");
